...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .subtotal-row { background: #dcfce7 !important; border-top: 1px solid #86efac !important; }
    .subtotal-row td { font-weight: 600; color: #166534; font-size: 12px; padding: 8px 12px; }
...
<div class="header">
    ...
    <div class="header-text">
        ...
        <div class="report-title">Depot Loading Report</div>
    </div>
</div>
...
<div class="meta">
    ...
    <div class="meta-item">
        <label>Total Drinks</label>
        <p>{{ $entries->count() }} drink(s)</p>
    </div>
    <div class="meta-item">
        <label>Total Crates</label>
        <p>{{ $entries->sum('crates') }} crates</p>
    </div>
    <div class="meta-item">
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Drink Name</th>
                <th class="right" style="width:130px">Crates Loaded</th>
                <th class="right" style="width:180px">Stock Value (GHS)</th>
            </tr>
        </thead>
        <tbody>
            @if($entries->isEmpty())
                <tr><td colspan="4" class="no-data">No stock loading recorded for this date.</td></tr>
            @else
                @foreach($entries as $i => $row)
                <tr>
                    <td>{{ $i + 1 }}</td>
                    <td><strong>{{ $row['drink_name'] }}</strong></td>
                    <td class="right">{{ number_format($row['crates']) }}</td>
                    <td class="right">GHS {{ number_format($row['total'], 2) }}</td>
                </tr>
                @endforeach
                <tr class="totals-row">
                    <td colspan="2">GRAND TOTAL</td>
                    <td class="right">{{ number_format($entries->sum('crates')) }} crates</td>
...
<div class="footer">
    <div class="footer-left">Obamax Gardens Ltd &mdash; Depot Loading Report &mdash; {{ $date }}</div>
